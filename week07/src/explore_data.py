...
def collect_stats(records: list) -> dict:
    entity_type_counts = Counter() # 按实体类型分类统计的数量字典
    entity_lengths = []            # 真实标签里的实体数量
    text_lengths = []              # 文本长度列表
    entity_per_sentence = []       # 每句话里有多少个实体
    entities_by_type = {}          # 按实体类型分组
    ...
    for row in records:
        ...
        entity_per_sentence.append(sent_entities)
