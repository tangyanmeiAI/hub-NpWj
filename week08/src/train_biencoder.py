...
适配设备：H3CBook Ultra 14T G3 Core Ultra7 165H + Arc核显 Windows原生（无WSL）
优化点：XPU原生加速、内存控压、CPU多线程、轻薄本低显存适配、无IPEX依赖
...
from torch.amp import autocast, GradScaler  # 混合精度，XPU同样兼容
...
def train_one_epoch_cosine(model, loader, optimizer, scheduler, device,
                           epoch, total_epochs, margin, grad_accum, scaler):
...
def main():
    ...
    # 【关键优化】原生XPU优先，不再依赖CUDA/IPEX
    if torch.xpu.is_available():
        device = torch.device("xpu")
        print(f"✅ 识别到 Intel Arc XPU 核显，使用XPU加速")
        scaler = GradScaler("xpu")
    else:
        device = torch.device("cpu")
        scaler = GradScaler("cpu")
        print(f"⚠️ XPU不可用，降级使用纯CPU训练")
        
    print(f"设备: {device}")
    print(f"Loss 类型: {args.loss}  池化策略: {args.pool}  "
          f"BERT 层数: {args.num_hidden_layers}  Epochs: {args.epochs}")
    print(f"BatchSize:{args.batch_size} 梯度累积:{args.grad_accum} 等效批次:{args.batch_size * args.grad_accum}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.loss == "cosine":
            train_loss = train_one_epoch_cosine(
                ...
                epoch, args.epochs, args.margin, args.grad_accum,scaler
            )
        else:
