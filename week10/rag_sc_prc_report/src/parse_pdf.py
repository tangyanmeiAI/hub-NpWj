"""
PDF 解析脚本：将四川移动资费标准 PDF 转换为结构化文本

教学重点（企业级 RAG 的真实挑战）：
  1. 数字 PDF vs 扫描件：处理方式完全不同
  2. 表格提取：资费表大量行列数据，直接按文字流提取会乱序
  3. 页眉/页脚噪声：每页可能有标题重复、页码，必须去除
...
class ParsedBlock:
    """
    一个解析块 = 资费标准文档里的一段连续内容（文字段落 or 表格）

    保留 page_num 和 section_path 非常重要——
    RAG 答案引用时能告诉用户"来自第2页，五、全家享融合套餐"
    """
    block_type:   str            # "text" | "table" | "title"
    content:      str            # 文字内容（表格转为 markdown）
    page_num:     int
    section_path: list[str]      # ["一、主力4G飞享套餐", "2. 全国统一飞享"]
...
NOISE_PATTERNS = [
    ...
    re.compile(r"^表格\s*$"),               # PDF 表格标记词
    re.compile(r"^中国移动\s*$"),           # 页眉/页脚公司名
]
...
class AnnualReportParser:
    ...
    def _update_section(self, title: str):
        ...
        if re.match(r"^第[一二三四五六七八九十]+[章节部分]", title):
            ...
            self._section_stack = self._section_stack[:1] + [title]  # 二级
