...
def chunk_semantic(
    ...
    注意：min_chunk_size 设为 50，因为资费文档中很多有效信息（如套餐名+价格）
         本身就很短，不能过滤掉。
    """
    buffer_blocks = []
    buffer_len    = 0
    ...
    # 判断"主标题"的模式：章节级标题才触发分块，子级标题不触发
    MAJOR_TITLE_RE = re.compile(r"^(第[一二三四五六七八九十百]+[章节部分]|[一二三四五六七八九十]+[、.])")

    def is_major_title(block: dict) -> bool:
        """判断是否是章节级主标题（如'一、xxx'、'第一部分xxx'），这些才触发分块。"""
        content = block.get("content", "").strip()
        return bool(MAJOR_TITLE_RE.match(content))

    for block in blocks:
        btype = block["block_type"]
        blen  = len(block["content"])

        # 主标题块：强制先 flush 之前的内容，然后将标题加入新缓冲区
        # （标题作为上下文前缀保留在 chunk 中，提升 embedding 匹配质量）
        # 子级标题（如"月租"、"1. xxx"）不触发分块，直接加入缓冲区
        if btype == "title" and is_major_title(block):
            if buffer_blocks:
...
def build_chunk_id(year: str, idx: int) -> str:
    return f"{year}_{idx:05d}"
...
def process_file(parsed_path: Path, strategy: str = STRATEGY):
    ...
    for idx, chunk in enumerate(raw_chunks):
        chunk_id = build_chunk_id(year, idx)
